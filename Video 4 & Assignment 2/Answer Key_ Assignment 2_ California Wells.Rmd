...
![](/cloud/project/Video 4 & Assignment 2/california_wells.jpg){width=450px}
...
ca_wells_long_records <- ca_wells %>% # Create a new data frame from our original data file 
  ...
  mutate(min_date = min(year), # Create a new column using the function `mutate()`: for minimum and maximum years 
         max_date = max(year)) %>% 
  filter(max_date >= 2010, # Use the `filter()` function to sort by minimum and maximum years for each site ID (well)
         min_date <= 1960) # We want any wells that have measurements from 1950 or before, and 2015 or later (long term) 
...
ca_wells_median <- ca_wells_long_records %>%  
  group_by(site_id, year) %>%
  mutate(water_level_median = median(water_level)) %>% 
  distinct(site_id, year, .keep_all = TRUE) %>% 
  ungroup() 
...
ca_wells_final <- ca_wells_median %>%  
  group_by(site_id) %>% 
  filter(n() >= 80) 
...
long_term_wells <- unique(ca_wells_final$site_id)
long_term_wells
...
mahsun_plot <- ca_wells_final %>% 
  filter(site_id == "340535117573501")

ggplot(data = mahsun_plot, 
       aes(x = year, 
  ...
  geom_line() + 
  scale_y_reverse()+
  scale_x_continuous(limits = c(1931, 2021), 
                     expand = c(0, 0), breaks = c(1931, 1950, 1975, 2000, 2021)) +
...
ggplot(data = ca_wells_final, aes(x= year, y= water_level_median, fill= site_id)) + 
    geom_area() + 
  scale_y_reverse() 
...
2. Once you have picked your wells, copy their site_id number. Make a new data frame (called 'my_ca_wells' or something similar) and use the `filter()` function to filter your chosen sites so the data frame only contains your chosen wells. Example code here: (filter(site_id %in% c("site#", "site#")))
